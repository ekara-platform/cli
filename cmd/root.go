...
func initLocalEngine(workDir string, descriptorURL string) engine.Ekara {
    ...
    repo, err := buildRepositoryFromCli(descriptorURL)
    if err != nil {
        common.Logger.Fatalf(common.INVALID_DESCRIPTOR_URL, err.Error())
...
func buildRepositoryFromCli(descriptorUrl string) (componentizer.Repository, error) {
    // Repository location
    ref := "master"
    split := strings.Split(descriptorUrl, "@")
    if len(split) > 1 {
        descriptorUrl = split[0]
        ref = split[1]
    }

    // Optional repository authentication
    auth := make(map[string]string)
    if common.Flags.Descriptor.Login != "" || common.Flags.Descriptor.Password != "" {
        auth["method"] = "basic"
        auth["user"] = common.Flags.Descriptor.Login
        auth["password"] = common.Flags.Descriptor.Password
    }

    return componentizer.CreateRepository(descriptorUrl, ref, auth)
}
